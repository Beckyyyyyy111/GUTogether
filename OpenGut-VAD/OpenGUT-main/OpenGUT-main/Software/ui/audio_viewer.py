...
from .components.playback_helpers import PlayheadOverlayManager, PlaybackControlsManager, format_mm_ss, QMediaPlayer, QAudioOutput, QMediaDevices
from const import (
    BUTTON_AUTO_RANGE,
    BUTTON_REPLOT,
    COLORBAR_PRESET,
...
class AudioViewerWidget(QWidget):
    ...
    def set_filter_preview_output(self, output_signal):
        ...
        if output_signal is not None and QMediaPlayer is not None:
            ...
            # Initialize processed player if not already created. This needs its
            # own QAudioOutput -- reusing self.playback_controls.audio_output here
            # would silently detach it from the original player (a QAudioOutput
            # can only be bound to one QMediaPlayer at a time), leaving "Original"
            # playback muted after a filter is applied.
            if self.processed_player is None:
                try:
                    if QAudioOutput is not None:
                        self.processed_audio_output = QAudioOutput(self)
                        self.processed_audio_output.setMuted(False)
                        self.processed_audio_output.setVolume(1.0)
                        self.processed_player = QMediaPlayer(self)
                        self.processed_player.setAudioOutput(self.processed_audio_output)
