...
from filters.speech_removal_filter import _resample_linear
from audio_moments import build_moments_clip, find_clean_candidates, DEFAULT_SELECTED_SOUNDS_DIR
from const import (
    ...
    MSG_FAILED_BUILD_PLAYBACK_CLIP,
    ...
    TEXT_PLAYBACK_CLIP_BUILT_STATUS,
    TEXT_PLAYBACK_CLIP_STATUS_IDLE,
    TEXT_PROGRESS_BUILDING_FILTER_OUTPUT,
    TEXT_PROGRESS_FINDING_MOMENTS,
    TEXT_PROGRESS_PREPARING_FILTER_INPUTS,
    TEXT_PROGRESS_WRITING_LOGS,
    TEXT_PROGRESS_WRITING_PLAYBACK_CLIP,
    TIMESTAMP_FORMAT,
...
def parse_raw_filename_tokens(path):
    """Parse a 'Sound_<timestamp>_<group>_<id>.wav' filename into its parts.

    Returns (timestamp, group, participant_id) or None if the stem does not
    follow this convention (e.g. legacy files, or already-derived outputs).
    """
    if not path:
        return None
    parts = Path(path).stem.split("_")
    if len(parts) == 4 and parts[0].lower() == "sound":
        _, timestamp, group, participant_id = parts
        return timestamp, group, participant_id
    return None
...
class PlaybackClipWorker(QObject):
    progress = pyqtSignal(int, str)
    finished = pyqtSignal(object)
    error = pyqtSignal(str)
    ...
    def run(self):
        try:
            self.progress.emit(20, TEXT_PROGRESS_FINDING_MOMENTS)
            combined, moments = build_moments_clip(self.signal, self.sr)
            self.progress.emit(80, TEXT_PROGRESS_WRITING_PLAYBACK_CLIP)
            self.finished.emit({"combined": combined, "moments": moments})
        except Exception as exc:
            self.error.emit(str(exc))


class CleanMomentsWorker(QObject):
    """Fallback tool for the rare recording where a participant didn't leave
    quiet gaps on their own: runs Silero VAD to find speech, then ranks the
    loudest speech-free windows as playback-clip candidates (see
    audio_moments.find_clean_candidates). Energy is a proxy for activity,
    not a gut-sound classifier -- results are candidates to spot-check by
    ear, not a fully-automated pick."""

    progress = pyqtSignal(int, str)
    finished = pyqtSignal(object)
    error = pyqtSignal(str)

    def __init__(self, signal, sr, duration_s, top_n, vad_threshold, vad_pad_ms):
        super().__init__()
        self.signal = np.asarray(signal, dtype=np.float32)
        self.sr = int(sr)
        ...
        self.vad_pad_ms = int(vad_pad_ms)
    ...
    def run(self):
        try:
            ...
            self.progress.emit(10, "Loading Silero VAD model")
            ...
            self.progress.emit(35, "Detecting speech")
            vad_audio = _resample_linear(self.signal, self.sr, 16000)
            segs = get_speech_timestamps(
                torch.from_numpy(vad_audio), model,
                sampling_rate=16000, threshold=self.vad_threshold,
                speech_pad_ms=self.vad_pad_ms, return_seconds=True,
            )
            speech_segments = [(s["start"], s["end"]) for s in segs]

            self.progress.emit(70, "Ranking speech-free windows")
            candidates = find_clean_candidates(
                self.signal, self.sr, speech_segments,
                duration_s=self.duration_s, top_n=self.top_n,
            )
            ...
            self.finished.emit({
                ...
                "total_s": len(self.signal) / self.sr,
            })
        except Exception as exc:
            self.error.emit(str(exc))
...
class FullPipelineWorker(QObject):
    """Runs the recommended filter order in one go -- Dual-Mic Noise
    Cancellation, then Speech Removal (VAD), then Scipy Highpass -- each
    step's output feeding the next, instead of three separate Apply Filter
    / Swap Source cycles. Uses each filter's own default parameters; run
    the individual filters manually instead if you need to tune one step.

    Dual-Mic runs first and only, against the original stereo pair, because
    it needs a real, untouched reference channel -- see
    dual_mic_noise_cancellation_filter.py. The later steps only ever look
    at the single channel being cleaned.
    """
    ...
    PIPELINE_STEPS = (
        "dual_mic_noise_cancellation_filter",
        "speech_removal_filter",
        "scipy_highpass_filter",
    )

    def __init__(self, loaded_filters, audio, sr, channel_index, source_path, output_dir):
        super().__init__()
        self.loaded_filters = list(loaded_filters)
        self.audio = np.asarray(audio, dtype=np.float32)
        self.sr = int(sr)
        self.channel_index = int(channel_index)
    ...
    def _find(self, file_stem):
        for loaded in self.loaded_filters:
            if loaded.file_stem == file_stem:
                return loaded
        return None
    ...
    def run(self):
        try:
            missing = [s for s in self.PIPELINE_STEPS if self._find(s) is None]
            if missing:
                raise RuntimeError(f"Missing filter module(s): {', '.join(missing)}")
            ...
            for i, file_stem in enumerate(self.PIPELINE_STEPS):
                loaded = self._find(file_stem)
                label = getattr(loaded.instance, "name", file_stem)
                step_labels.append(label)
                base_pct = int(i * (90 / n_steps))
                self.progress.emit(base_pct, f"Step {i + 1}/{n_steps}: {label}")

                schema = loaded.instance.get_parameter_schema()
                params = {p["key"]: p["default"] for p in schema}

                apply_signature = inspect.signature(loaded.instance.apply)
                apply_kwargs = {}
                if "progress_callback" in apply_signature.parameters:
                    def sub_progress(value, detail_text="", _i=i, _n=n_steps, _label=label):
                        mapped = int(_i * (90 / _n) + (value / 100) * (90 / _n))
                        self.progress.emit(mapped, f"Step {_i + 1}/{_n}: {_label} -- {detail_text}")
                    apply_kwargs["progress_callback"] = sub_progress

                output_channel = loaded.instance.apply(
                    current_audio, self.sr, current_channel_index, params,
                    self.source_path, self.output_dir, **apply_kwargs,
                )
                target_len = current_audio.shape[-1] if current_audio.ndim > 1 else len(current_audio)
                output_channel = normalize_signal_length(output_channel, target_len)

                # Feed this step's output as the single target channel for
                # the next step -- only Dual-Mic (already run, above) needs
                # a second, independent reference channel.
                current_audio = np.asarray([output_channel], dtype=np.float32)
                current_channel_index = 0

            self.progress.emit(95, "Finalizing")
            self.finished.emit({
                "normalized": current_audio[0],
                "channel_index": self.channel_index,
                "params": {"pipeline": " -> ".join(step_labels)},
                "file_stem": "full_pipeline",
                "output_dir": self.output_dir,
            })
        except Exception as exc:
            self.error.emit(str(exc))
...
class AnnotationPanelWidget(QWidget):
    ...
    def __init__(self):
        ...
        self.recording_tokens = None  # (timestamp, group, participant_id) parsed from raw filename
    ...
    def init_ui(self):
        ...
        self.run_full_pipeline_btn = QPushButton(
            "Run Full Pipeline (Dual-Mic → VAD → Highpass)"
        )
        self.run_full_pipeline_btn.setToolTip(
            "Runs all three recommended filters in order, each step feeding the "
            "next, using each filter's default parameters. Result lands in "
            "Processed Output same as a single Apply Filter -- click Swap Source "
            "to keep it, same as usual."
        )
        self.run_full_pipeline_btn.clicked.connect(self.run_full_pipeline)
        filtering_tab_layout.addWidget(self.run_full_pipeline_btn)
        ...
        # Playback clip builder: joins the during-eating/near-end-of-eating/peak
        # moments (see audio_moments.py) into one WAV for participant playback.
        playback_title = QLabel(SECTION_PLAYBACK_CLIP)
        playback_title.setStyleSheet(SECTION_TITLE_STYLE)
        filtering_tab_layout.addWidget(playback_title)

        participant_row = QHBoxLayout()
        participant_row.setSpacing(6)
        participant_row.addWidget(QLabel(LABEL_PARTICIPANT_ID))
        self.participant_id_input = QLineEdit()
        participant_row.addWidget(self.participant_id_input, 1)
        filtering_tab_layout.addLayout(participant_row)

        self.build_playback_clip_btn = QPushButton(BUTTON_BUILD_PLAYBACK_CLIP)
        self.build_playback_clip_btn.clicked.connect(self.build_playback_clip)
        filtering_tab_layout.addWidget(self.build_playback_clip_btn)

        self.playback_clip_status_label = QLabel(TEXT_PLAYBACK_CLIP_STATUS_IDLE)
        self.playback_clip_status_label.setWordWrap(True)
        filtering_tab_layout.addWidget(self.playback_clip_status_label)

        # Fallback tool for the rare recording where a participant didn't
        # self-manage quiet gaps: VAD finds the speech to avoid, then ranks
        # the loudest speech-free windows as candidates to spot-check by ear.
        clean_moments_title = QLabel("Find Clean Moments (VAD fallback)")
        clean_moments_title.setStyleSheet(SECTION_TITLE_STYLE)
        filtering_tab_layout.addWidget(clean_moments_title)

        clean_moments_row = QHBoxLayout()
        clean_moments_row.setSpacing(6)
        clean_moments_row.addWidget(QLabel("Window (s)"))
        self.clean_moments_duration_spin = QDoubleSpinBox()
        self.clean_moments_duration_spin.setRange(1.0, 600.0)
        self.clean_moments_duration_spin.setValue(60.0)
        clean_moments_row.addWidget(self.clean_moments_duration_spin)
        clean_moments_row.addWidget(QLabel("Top N"))
        self.clean_moments_topn_spin = QSpinBox()
        self.clean_moments_topn_spin.setRange(1, 50)
        self.clean_moments_topn_spin.setValue(10)
        clean_moments_row.addWidget(self.clean_moments_topn_spin)
        clean_moments_row.addWidget(QLabel("VAD Sensitivity"))
        self.clean_moments_threshold_spin = QDoubleSpinBox()
        self.clean_moments_threshold_spin.setRange(0.05, 0.95)
        self.clean_moments_threshold_spin.setSingleStep(0.05)
        self.clean_moments_threshold_spin.setValue(0.3)
        clean_moments_row.addWidget(self.clean_moments_threshold_spin)
        clean_moments_row.addStretch()
        filtering_tab_layout.addLayout(clean_moments_row)

        self.find_clean_moments_btn = QPushButton("Find Clean Moments")
        self.find_clean_moments_btn.clicked.connect(self.find_clean_moments)
        filtering_tab_layout.addWidget(self.find_clean_moments_btn)

        self.clean_moments_status_label = QLabel(
            "No search run yet. Double-click a result below to jump the Annotation "
            "tab's Start/Stop to that window."
        )
        self.clean_moments_status_label.setWordWrap(True)
        filtering_tab_layout.addWidget(self.clean_moments_status_label)

        self.clean_moments_list = QListWidget()
        self.clean_moments_list.setMaximumHeight(160)
        self.clean_moments_list.itemDoubleClicked.connect(self.on_clean_moment_selected)
        filtering_tab_layout.addWidget(self.clean_moments_list)
    ...
    def _build_filter_output_stem(self, filter_file_stem, channel_index, unix_time):
        ...
        if self.recording_tokens:
            timestamp, group, participant_id = self.recording_tokens
            return f"sound_{filter_stem}_{timestamp}_{side}_{group}_{participant_id}"
    ...
    def run_full_pipeline(self):
        """Run Dual-Mic Noise Cancellation -> Speech Removal (VAD) -> Scipy
        Highpass in one go instead of three manual Apply Filter / Swap
        cycles. Lands in Processed Output exactly like a single filter
        apply -- use the existing Swap Source button to keep the result."""
        if self.current_audio is None or self.current_sr is None:
            QMessageBox.warning(self, MSG_WARNING_TITLE, MSG_LOAD_AUDIO_BEFORE_FILTER)
            return

        if self.is_mono or self.current_audio.ndim == 1:
            QMessageBox.warning(
                self, MSG_WARNING_TITLE,
                "Run Full Pipeline needs stereo input (Dual-Mic Noise Cancellation's "
                "first step needs both microphone channels).",
            )
            return

        missing = [s for s in FullPipelineWorker.PIPELINE_STEPS
                   if not any(loaded.file_stem == s for loaded in self.loaded_filters)]
        if missing:
            QMessageBox.critical(
                self, MSG_FILTER_ERROR_TITLE,
                f"Missing filter module(s): {', '.join(missing)}",
            )
            return
    ...
    def build_playback_clip(self):
        """Locate the during-eating/near-end-of-eating/peak moments in the
        current audio and save them, joined by silence, to Selected_Sounds."""
        if self.current_audio is None or self.current_sr is None:
            QMessageBox.warning(self, MSG_WARNING_TITLE, MSG_LOAD_AUDIO_BEFORE_FILTER)
            return
        ...
        if self.recording_tokens:
            timestamp, group, participant_id = self.recording_tokens
            output_name = f"selected_sound_{timestamp}_{side}_{group}_{participant_id}.wav"
        else:
            participant_text = sanitize_name_part(self.participant_id_input.text().strip())
            if not participant_text or participant_text == "unknown":
                QMessageBox.warning(self, MSG_WARNING_TITLE, MSG_PARTICIPANT_ID_REQUIRED)
                return
            output_name = f"{participant_text}_playback_clip.wav"

        if self.is_mono or self.current_audio.ndim == 1:
            signal = np.asarray(self.current_audio, dtype=np.float32).flatten()
        else:
            signal = np.asarray(self.current_audio[channel_index], dtype=np.float32)

        if self.run_background_task_callback is None:
            QMessageBox.critical(
                self, MSG_FILTER_ERROR_TITLE,
                MSG_FAILED_BUILD_PLAYBACK_CLIP.format(error="Background task runner is not configured."),
            )
            return

        self._pending_playback_clip_name = output_name
        worker = PlaybackClipWorker(signal, self.current_sr)
        self.run_background_task_callback(worker, PLAYBACK_CLIP_BUSY_TITLE, PLAYBACK_CLIP_BUSY_TITLE, self.on_playback_clip_built)

    def on_playback_clip_built(self, result):
        """Save the combined clip on the UI thread after the backend worker finishes."""
        try:
            combined = np.asarray(result["combined"], dtype=np.float32)
            moments = result["moments"]
            output_name = getattr(self, "_pending_playback_clip_name", None) or "playback_clip.wav"

            DEFAULT_SELECTED_SOUNDS_DIR.mkdir(parents=True, exist_ok=True)
            output_path = DEFAULT_SELECTED_SOUNDS_DIR / output_name
            sf.write(str(output_path), combined, int(self.current_sr))

            details = "; ".join(f"{moment['label']}={moment['start']:.1f}s" for moment in moments)
            total_s = len(combined) / self.current_sr
            self.playback_clip_status_label.setText(
                TEXT_PLAYBACK_CLIP_BUILT_STATUS.format(name=output_path.name, total=total_s, details=details)
            )
        except Exception as exc:
            QMessageBox.critical(self, MSG_FILTER_ERROR_TITLE, MSG_FAILED_BUILD_PLAYBACK_CLIP.format(error=exc))

    def find_clean_moments(self):
        """Fallback tool: rank the loudest speech-free windows in the current
        audio as playback-clip candidates. See CleanMomentsWorker."""
        if self.current_audio is None or self.current_sr is None:
            QMessageBox.warning(self, MSG_WARNING_TITLE, MSG_LOAD_AUDIO_BEFORE_FILTER)
            return
        ...
        if self.run_background_task_callback is None:
            QMessageBox.critical(
                self, MSG_FILTER_ERROR_TITLE,
                "Background task runner is not configured.",
            )
            return

        self.clean_moments_list.clear()
        self.clean_moments_status_label.setText("Searching...")

        worker = CleanMomentsWorker(
            signal, self.current_sr,
            duration_s=self.clean_moments_duration_spin.value(),
            top_n=self.clean_moments_topn_spin.value(),
            vad_threshold=self.clean_moments_threshold_spin.value(),
            vad_pad_ms=150,
        )
        self.run_background_task_callback(worker, "Finding Clean Moments", "Finding Clean Moments", self.on_clean_moments_found)

    def on_clean_moments_found(self, result):
        """Populate the candidate list on the UI thread after the backend worker finishes."""
        try:
            ...
            self.clean_moments_list.clear()
            if not candidates:
                self.clean_moments_status_label.setText(
                    f"No speech-free window of this length found ({len(speech_segments)} speech "
                    f"segment(s), {total_speech:.1f}s of {total_s:.1f}s total). Try a shorter window."
                )
                return

            for c in candidates:
                item = QListWidgetItem(
                    f"{c['start']:.1f}s - {c['end']:.1f}s   (rms={c['rms']:.4f}, {c['db']:.1f} dBFS)"
                )
                item.setData(Qt.ItemDataRole.UserRole, (c["start"], c["end"]))
                self.clean_moments_list.addItem(item)

            self.clean_moments_status_label.setText(
                f"{len(candidates)} candidate(s) found. {len(speech_segments)} speech segment(s) "
                f"excluded ({total_speech:.1f}s of {total_s:.1f}s total). Double-click a result to "
                f"jump the Annotation tab's Start/Stop there -- listen before using it."
            )
        except Exception as exc:
            QMessageBox.critical(self, MSG_FILTER_ERROR_TITLE, f"Failed to find clean moments: {exc}")

    def on_clean_moment_selected(self, item):
        """Jump the Annotation tab's Start/Stop selection to the double-clicked candidate."""
        start, end = item.data(Qt.ItemDataRole.UserRole)
        self.tab_widget.setCurrentIndex(0)  # Annotation tab
    ...
    def set_audio_data(self, y, sr, is_mono=False, file_path=None):
        ...
        self.recording_tokens = parse_raw_filename_tokens(file_path)
        if self.recording_tokens:
            _, group, participant_id = self.recording_tokens
            self.participant_id_input.setText(f"{group}_{participant_id}")
        else:
