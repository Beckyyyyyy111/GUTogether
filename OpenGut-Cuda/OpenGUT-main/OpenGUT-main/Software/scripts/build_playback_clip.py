...
def main():
    ...
    parser.add_argument("--eating-end-offset", type=float, default=DEFAULT_EATING_END_OFFSET,
                         help="Seconds before the recording ends where the 'near end of eating' moment starts (default: 360 / 6min).")
    parser.add_argument("--peak-search-margin", type=float, default=DEFAULT_PEAK_SEARCH_MARGIN,
                         help="Seconds excluded from the start and end of the recording when searching for the "
                              "'peak gut-sound activity' moment, to avoid device donning/removal noise (default: 180 / 3min).")
    ...
    combined, moments = build_moments_clip(
        ...
        peak_search_margin=args.peak_search_margin,
        overrides=overrides,
    )
