...
def _build_audiosep_model():
    ...
    try:
        ...
        # numpy 2.x moved numpy.core -> numpy._core internally, so the pickled
        # qualname inside these checkpoints doesn't match what gets registered
        # through the numpy.core compat shim above -- add_safe_globals alone
        # isn't enough on numpy>=2. These checkpoints are AudioSep's own,
        # linked from AudioSep/README.md's official Hugging Face repo, so
        # disabling weights_only for them is safe here.
        if not getattr(torch.load, "_weights_only_false_patch", False):
            ...
            _torch_load_full._weights_only_false_patch = True
